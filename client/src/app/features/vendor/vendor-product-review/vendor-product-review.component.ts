...
export class VendorProductReviewComponent implements OnInit {
      ...
      next: (product: Product) => {
        ...
        if (product.pictureUrl) {
          ...
          if (product.pictureUrl.startsWith('http://') || product.pictureUrl.startsWith('https://')) {
            ...
            // If it's a relative path, construct the full URL using environment config
  ...
  onImageError(event: any): void {
    ...
    if (this.imagePreview && !this.imagePreview.includes('data:')) {
      // Try HTTP instead of HTTPS
      if (this.imagePreview.includes('https://localhost:5001')) {
        this.imagePreview = this.imagePreview.replace('https://localhost:5001', 'http://localhost:5000');
      }
      // Try without /images/ prefix
      else if (this.imagePreview.includes('/images/')) {
        this.imagePreview = this.imagePreview.replace('/images/', '/');
      }
      // Try with /images/ prefix
      else if (!this.imagePreview.includes('/images/')) {
        this.imagePreview = this.imagePreview.replace('/products/', '/images/products/');
      }
      
      console.log('Trying alternative URL:', this.imagePreview);
      event.target.src = this.imagePreview;
      
      // If this also fails, try placeholder as final fallback
      setTimeout(() => {
        if (event.target.naturalWidth === 0) {
          console.log('Alternative URL also failed, trying placeholder...');
          const baseUrl = environment.baseUrl.replace('/api/', '');
